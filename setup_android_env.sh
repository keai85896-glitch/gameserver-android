...
replace_aapt2() {
  ...
  # 替换 Gradle 缓存中 Maven 下载的 x86 aapt2 jar 为 ARM64 版本
  local gradle_aapt_jars
  gradle_aapt_jars=$(find "$gradle_cache_root/modules-2" -name "aapt2-*-linux.jar" -type f 2>/dev/null || true)
  if [[ -n "$gradle_aapt_jars" ]]; then
    while IFS= read -r jar_path; do
      local jar_dir
      jar_dir=$(dirname "$jar_path")
      # 检查 jar 中 aapt2 是否已是 ARM64
      local tmp_check
      tmp_check=$(mktemp -d)
      unzip -o -q "$jar_path" aapt2 -d "$tmp_check" 2>/dev/null || true
      if [[ -f "$tmp_check/aapt2" ]]; then
        local is_arm64
        is_arm64=$(file "$tmp_check/aapt2" 2>/dev/null | grep -c "ARM aarch64" || true)
        if [[ "$is_arm64" -eq 0 ]]; then
          log "Patching x86 aapt2 in Maven cache: $jar_path"
          cp "$aapt2_path" "$tmp_check/aapt2"
          chmod +x "$tmp_check/aapt2"
          (cd "$tmp_check" && zip -0 -q "$jar_path" aapt2)
        fi
      fi
      rm -rf "$tmp_check"
    done <<< "$gradle_aapt_jars"
  fi
